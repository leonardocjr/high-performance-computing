...
{    
    ...
    for(t=0;t<NUM_THREADS;t++){
        ...
        if (slice_width > 0) { // NUM_CELLS >= NUM_THREADS, i.e., n>=w			
        ...
        else { // NUM_CELLS < NUM_THREADS, i.e., n<w
            // 1st NUM_CELLS threads will be busy with a single dot product
            if (t<NUM_CELLS) GLOBAL_slices[t].end=GLOBAL_slices[t].begin=t;
            // remaining threads will be idle (they will still be created
            // but their loop will not execute and will return imediatelly)
            else { GLOBAL_slices[t].end=-2; GLOBAL_slices[t].begin=-1; }
        }
