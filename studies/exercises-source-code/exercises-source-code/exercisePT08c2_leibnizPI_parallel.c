...
typedef struct {
    double sum;                // 8 bytes
    unsigned char padding[56]; // 56 bytes
} sum_t;                       // 64 bytes (1 cache line)
sum_t GLOBAL_sum[NUM_WORKERS];
...
void *aThread(void *arg)
{
 ...
 unsigned long k;
 double factor = 1.0;
 unsigned long begin=GLOBAL_slices[tid].begin, end=GLOBAL_slices[tid].end;
 
 printf("thread %lu: begin = %lu end = %lu\n", tid, begin, end);
 if (begin % 2 != 0) factor = -1.0;
 for (k=begin; k<=end; k++) {
   GLOBAL_sum[tid].sum += factor/(2*k+1);
   factor = -factor;
 }
